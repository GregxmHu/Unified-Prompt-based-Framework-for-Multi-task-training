        ...
        for step, train_batch in enumerate(train_loader):
            ...
            if (step+1) % args.gradient_accumulation_steps == 0:
                ...
                if (global_step+1) % args.eval_every == 0 or (args.test_init_log and global_step==0):                
                    model.eval()
                    with torch.no_grad():
                        total,right=dev(args, model, dev_loader, device)
                    model.train()
...
def main():
    ...
    train_set=MNLIDataset(dataset=args.train,ckpt=args.vocab)
    logger.info('reading dev data...')
    dev_set=MNLIDataset(dataset=args.dev,ckpt=args.vocab)
